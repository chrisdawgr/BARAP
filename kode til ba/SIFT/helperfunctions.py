...
def drawMatches(I1, kp1, I2, kp2, matches):
  ...
  for mat in range(0, len(kp1)):
    ...
    # colour blue
    # thickness = 1
    cv2.circle(out, (int(x1),int(y1)), math.sqrt(2) * int(scale1), color1, 3)   
    cv2.circle(out, (int(x2)+cols1,int(y2)), math.sqrt(2) * int(scale2), color2, 3)
...
def transform_coordinates(f_list):
  ...
  for ele in f_list:
    ...
    desc = ele[1]
    if (ele[0][2] == 5 or ele[0][2] == 6):
      point = [ele[0][0] * 2, ele[0][1] * 2, ele[0][2], ele[0][3]]
    if (ele[0][2] == 10 or ele[0][2] == 11):
      point = [ele[0][0] * 4, ele[0][1] * 4, ele[0][2], ele[0][3]]
    if (ele[0][2] == 15 or ele[0][2] == 16):
      point = [ele[0][0] * 8, ele[0][1] * 8, ele[0][2], ele[0][3]]
    if (ele[0][2] == 20 or ele[0][2] == 21):
      point = [ele[0][0] * 16, ele[0][1] * 16, ele[0][2], ele[0][3]]
...
def points_ok(pp, sift_or_surf):
  """
  input: npy points, 0 = SIFT, 1 = SURF
  ...
  """
  points_not_matched_1 = np.zeros(25)

  if(sift_or_surf == 0):
    for i in range(0, len(pp)):
      points_not_matched_1[int(pp[i, 2])] += 1

  else:
    for i in range(0, len(pp)):
      if(pp[i,2] > 9 and pp[i,2] < 11):
        points_not_matched_1[16] += 1
      if(pp[i,2] > 6 and pp[i,2] < 7):
        points_not_matched_1[17] += 1

      if (pp[i,2] > 5 and pp[i,2] < 6):
        points_not_matched_1[16] += 1
      if (pp[i,2] > 3 and pp[i,2] < 4):
        points_not_matched_1[17] += 1

      if (pp[i,2] > 15 and pp[i,2] < 18):
        points_not_matched_1[16] += 1
      if (pp[i,2] > 20 and pp[i,2] < 23):
        points_not_matched_1[17] += 1

  #print(points_not_matched_1)
  #print((points_not_matched_1[12]))
